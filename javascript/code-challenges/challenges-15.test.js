...
const screenForNames = (arr) => {
  ...
  return arr.filter((name) => {
    const nameScreened = name.split(' ');
    return (
      !name.startsWith(' ') &&
      regex.test(name) &&
      !(nameScreened.length > 2 && nameScreened[1] === '')
    );
  });
};
...
let biggerThanLuke = (arr) => {
  ...
  const results = heavierCharacters.map(character => character.name).join(' - ');
  return results;
};
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  const detectTicTacToeWin = (broad) => {
    const check = (row1, col1, row2, col2, row3, col3) => {
      return board[row1][col1] === broad[row2][col2] && board[row2][col2] === board[row3][col3] && board[row1][col1] !== '';
    };
